  ...
  asyncWrapper(async (req, res) => {
    ...
    if (object === "page") {
      entry.forEach(async (entry: { messaging: FBMessaging[] }) => {
        const { sender, recipient, message, referral } = entry.messaging[0];
        console.log(entry.messaging[0]);

        if (!sender.id) return;
        if (recipient.id !== functions.config().fb.page_id) return;

        let userId = message?.text?.trim().split("認證碼:")[1];

        if (referral) {
          userId = referral.ref?.trim().split("認證碼:")[1];
        }
        ...
        if (!user) return;
        const { data } = await getFBPageUserProfile(sender.id);
        if (await isSameFBName(data, user.displayName)) {
          ...
          await sendFirstFBMessage(
            `${user.displayName}跟${data.last_name}${data.first_name}不符！請確認您Messenger登入的帳號！`,
            sender.id
          );
        }
      });
